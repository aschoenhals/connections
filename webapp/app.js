...
function buildAdjacency() {
  const adj = new Map(state.nodes.map(n => [n.person_id, new Set()]));
  for (const e of state.edges) {
    if (adj.has(e.from_id)) adj.get(e.from_id).add(e.to_id);
    if (adj.has(e.to_id))   adj.get(e.to_id).add(e.from_id);
  }
  return adj;
}

function sortedByDegree() {
  return [...state.nodes].sort((a, b) => b.degree - a.degree || a.display_name.localeCompare(b.display_name));
}

function bfsLevels(root, adj) {
  const levels = [];
  const visited = new Set([root.person_id]);
  let queue = [root];
  while (queue.length) {
    levels.push([...queue]);
    const next = [];
    for (const n of queue) {
      for (const pid of (adj.get(n.person_id) || [])) {
        if (!visited.has(pid)) {
          visited.add(pid);
          const node = state.nodeMap.get(pid);
  ...
  const disconnected = state.nodes.filter(n => !visited.has(n.person_id));
...
function applyLayout(type) {
  ...
  const c = screenToWorld({ x: canvas.clientWidth / 2, y: canvas.clientHeight / 2 });
  const cx = c.x, cy = c.y;
  switch (type) {
    case 'radial':    layoutRadial(cx, cy);       break;
    case 'tree':      layoutTree(cx, cy);         break;
    case 'org':       layoutOrg(cx, cy);          break;
    case 'fishbone':  layoutFishbone(cx, cy);     break;
    case 'bubble':    layoutBubble(cx, cy);       break;
    case 'flow':      layoutFlow(cx, cy);         break;
    case 'concept':   layoutConcept(cx, cy);      break;
    case 'double':    layoutDoubleBubble(cx, cy); break;
  }
  persist();
}

// 1. Radial / Spider – Most-connected node center, others in rings
function layoutRadial(cx, cy) {
  const sorted = sortedByDegree();
  sorted[0].x = cx; sorted[0].y = cy;
  const others = sorted.slice(1);
  let ring = 1, placed = 0;
  while (placed < others.length) {
    const count = Math.min(Math.round(ring * 6), others.length - placed);
    const r = ring * 115;
    for (let i = 0; i < count && placed < others.length; i++, placed++) {
      const angle = (Math.PI * 2 * i) / count - Math.PI / 2;
      others[placed].x = cx + Math.cos(angle) * r;
      others[placed].y = cy + Math.sin(angle) * r;
    }
    ring++;
  }
}

// 2. Tree / Hierarchical – BFS levels top-to-bottom
function layoutTree(cx, cy) {
  const adj = buildAdjacency();
  const levels = bfsLevels(sortedByDegree()[0], adj);
  const levelH = 130;
  const totalH = (levels.length - 1) * levelH;
  levels.forEach((level, i) => {
    const y = cy - totalH / 2 + i * levelH;
    const spacing = Math.min(160, 1200 / Math.max(level.length, 1));
    const totalW = (level.length - 1) * spacing;
    level.forEach((node, j) => {
      node.x = cx - totalW / 2 + j * spacing;
      node.y = y;
    });
  });
}

// 3. Org Chart – BFS levels, wider spacing, tighter vertical
function layoutOrg(cx, cy) {
  const adj = buildAdjacency();
  const levels = bfsLevels(sortedByDegree()[0], adj);
  const levelH = 110;
  const totalH = (levels.length - 1) * levelH;
  levels.forEach((level, i) => {
    const y = cy - totalH / 2 + i * levelH;
    const spacing = Math.min(190, 1400 / Math.max(level.length, 1));
    const totalW = (level.length - 1) * spacing;
    level.forEach((node, j) => {
      node.x = cx - totalW / 2 + j * spacing;
      node.y = y;
...
// 4. Fishbone / Ishikawa – Root right, branches alternate above/below spine
function layoutFishbone(cx, cy) {
  const adj = buildAdjacency();
  const sorted = sortedByDegree();
  const root = sorted[0];
  root.x = cx + 280; root.y = cy;

  const mainBoneIds = adj.get(root.person_id) || new Set();
  const mainBones = [...mainBoneIds].map(pid => state.nodeMap.get(pid)).filter(Boolean);
  const rest = sorted.slice(1).filter(n => !mainBoneIds.has(n.person_id));

  const allBones = [...mainBones, ...rest];
  allBones.forEach((node, i) => {
    const side = i % 2 === 0 ? -1 : 1;
    const step = Math.floor(i / 2);
    node.x = cx + 100 - step * 130;
    node.y = cy + side * 115;
  });
}

// 5. Bubble Map – Center node, all others in one ring
function layoutBubble(cx, cy) {
  const sorted = sortedByDegree();
  sorted[0].x = cx; sorted[0].y = cy;
  const others = sorted.slice(1);
  if (!others.length) return;
  const r = Math.max(150, others.length * 24);
...
// 6. Flow Map – Topological order left-to-right
function layoutFlow(cx, cy) {
  const inDeg = new Map(state.nodes.map(n => [n.person_id, 0]));
  const outAdj = new Map(state.nodes.map(n => [n.person_id, []]));
  for (const e of state.edges) {
    inDeg.set(e.to_id, (inDeg.get(e.to_id) || 0) + 1);
    outAdj.get(e.from_id)?.push(e.to_id);
  }
  const queue = state.nodes.filter(n => inDeg.get(n.person_id) === 0);
  const order = [];
  const visited = new Set();
  while (queue.length) {
    const n = queue.shift();
    if (visited.has(n.person_id)) continue;
    visited.add(n.person_id);
    order.push(n);
    for (const pid of outAdj.get(n.person_id) || []) {
      const nb = state.nodeMap.get(pid);
      if (nb && !visited.has(pid)) {
        inDeg.set(pid, inDeg.get(pid) - 1);
        if (inDeg.get(pid) === 0) queue.push(nb);
      }
    }
  }
  for (const n of sortedByDegree()) { if (!visited.has(n.person_id)) order.push(n); }
  const spacing = Math.min(150, 1400 / Math.max(order.length, 1));
  const totalW = (order.length - 1) * spacing;
  order.forEach((node, i) => {
    node.x = cx - totalW / 2 + i * spacing;
    node.y = cy;
  });
}

// 7. Concept Map – Grid spread, most connected nodes distributed
function layoutConcept(cx, cy) {
  const nodes = sortedByDegree();
  const cols = Math.ceil(Math.sqrt(nodes.length));
  const rows = Math.ceil(nodes.length / cols);
  const spacingX = 170, spacingY = 140;
  nodes.forEach((node, i) => {
    const col = i % cols;
    const row = Math.floor(i / cols);
    const rowCols = Math.min(cols, nodes.length - row * cols);
    node.x = cx + (col - (rowCols - 1) / 2) * spacingX;
    node.y = cy + (row - (rows - 1) / 2) * spacingY;
  });
}

// 8. Double Bubble – Two central nodes; left-only, right-only, shared groups
function layoutDoubleBubble(cx, cy) {
  const adj = buildAdjacency();
  const sorted = sortedByDegree();
  if (sorted.length < 2) { layoutBubble(cx, cy); return; }
  const left = sorted[0], right = sorted[1];
  const sep = 320;
  left.x  = cx - sep / 2; left.y  = cy;
  right.x = cx + sep / 2; right.y = cy;

  const leftPids  = adj.get(left.person_id)  || new Set();
  const rightPids = adj.get(right.person_id) || new Set();
  const leftOnly = [], rightOnly = [], shared = [], neither = [];
  for (const n of sorted.slice(2)) {
    const inL = leftPids.has(n.person_id), inR = rightPids.has(n.person_id);
    if (inL && inR) shared.push(n);
    else if (inL)   leftOnly.push(n);
    else if (inR)   rightOnly.push(n);
    else            neither.push(n);
  }
  // Unconnected → split equally
  neither.forEach((n, i) => (i % 2 === 0 ? leftOnly : rightOnly).push(n));

  leftOnly.forEach((n, i) => {
    const angle = (Math.PI / 2) + (Math.PI * (i + 1)) / (leftOnly.length + 1);
    n.x = left.x + Math.cos(angle) * 145;
    n.y = left.y + Math.sin(angle) * 145;
  });
  rightOnly.forEach((n, i) => {
    const angle = -(Math.PI * (i + 1)) / (rightOnly.length + 1);
    n.x = right.x + Math.cos(angle) * 145;
    n.y = right.y + Math.sin(angle) * 145;
  });
  shared.forEach((n, i) => {
    const angle = -Math.PI / 2 + (Math.PI * (i - (shared.length - 1) / 2)) / Math.max(shared.length, 1) * 0.5;
    n.x = cx + Math.sin(angle) * 60;
    n.y = cy - 110 - Math.floor(i / 3) * 80;
  });
}
...
document.addEventListener('pointerdown', e => {
  if (!layoutMenu.hidden && !layoutMenu.contains(e.target) && e.target !== resetBtn) {
